...
static int eng_led_green_test(int brightness) {
		...
		fd = open(LED_GREEN_DEV_K414,O_RDWR);
		if (fd < 0) {
			SPRD_DBG("%s: open %s fail", __func__, LED_GREEN_DEV_K414);
			return -1;
		}
        }
...
void backlight_init(void) {
	...
	fd = open(LED_GREEN_MAX_DEV_K414, O_RDONLY);
	if (fd < 0) {
		SPRD_DBG("%s: open %s fail", __func__, LED_GREEN_MAX_DEV_K414);
	} else {
		memset(buffer, 0, sizeof(buffer));
		ret = read(fd, buffer, sizeof(buffer));
		max_green_led = atoi(buffer);
		SPRD_DBG("max_green_led ====== %d\n",max_green_led);
		if (ret < 0) {
			SPRD_DBG("%s: read %s fail", __func__,LED_GREEN_MAX_DEV_K414);
		}
		close(fd);
	}
    } else {
	...
	fd = open(LED_RED_MAX_DEV_K414, O_RDONLY);
	if (fd < 0) {
		 SPRD_DBG("%s: open %s fail", __func__, LED_RED_MAX_DEV_K414);
	} else {
		memset(buffer, 0, sizeof(buffer));
		ret = read(fd, buffer, sizeof(buffer));
		max_red_led = atoi(buffer);
		SPRD_DBG("max_red_led ====== %d\n",max_red_led);
		if (ret < 0) {
			SPRD_DBG("%s: read %s fail", __func__, LED_RED_MAX_DEV_K414);
		}
		close(fd);
	}
    } else {
	...
	fd = open(LED_BLUE_MAX_DEV_K414, O_RDONLY);
	if (fd < 0) {
		SPRD_DBG("%s: open %s fail", __func__, LED_BLUE_MAX_DEV_K414);
	} else {
		memset(buffer, 0, sizeof(buffer));
		ret = read(fd, buffer, sizeof(buffer));
		max_blue_led = atoi(buffer);
		SPRD_DBG("max_blue_led ======%d\n",max_blue_led);
		if (ret < 0) {
			SPRD_DBG("%s: read %s fail", __func__, LED_BLUE_MAX_DEV_K414);
		}
		close(fd);
	}
    } else {
